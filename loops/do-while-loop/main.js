...
let i = 0;
while (i < 15){
   console.log(`this is the number is ${i}`);
   i++;
...
let names =["Berlin","Paris","Prague","Rome"];
let x = 0;
let text = "";
while(names[x]){
    text += names[x] + " ";
...
// #### 3. Odds and Evens
// Create a function that takes an array as an argument. The function should add 1 to each odd integer
// and should minus 1 from each even integer. 
// * Examples:
...
// * evenOddTransform([6, 9, 10, 20] -> expected output: [5, 10, 9, 19]
 ...
 function evenOddTransform (arrNumber){
    let h = 0
    while (h < arrNumber.length) {
        if (arrNumber[h] % 2 == 0) {
          arrNumber[h]--
        }
        else{
...
// #### 1. sumOfNumbers
// Create a function that takes an array of at least 3 numbers as an argument and 
// returns the sum of the numbers. Use a do while loop. 
let arrayOfNumbers = [];
let j = 0;
let sum = 0;
